...
if(!["764893492341112852","764893503934300160","764893501329637396","794285242974208020"].some(role => message.member.roles.cache.get(role)) && (!message.member.hasPermission("ADMINISTRATOR"))) 
return message.channel.send(new MessageEmbed().setDescription(`${message.author} <a:Cros:788055287768678421>**Bu Komutu Kullanabilmek İçin Yetkin Bulunmuyor.**<a:Cros:788055287768678421>`).setColor('0x800d0d').setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
  
const cezalırol = '764893527683104849'//Jail Rol 
const jaillog = message.guild.channels.cache.find(c => c.id === '787910698775740426')//Jail Log
