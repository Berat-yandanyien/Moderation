...
exports.run = async(client, message, args, ayar, emoji) => {
  ...
  if (!message.member.hasPermission("VIEV_AUDIT_LOG")) return message.channel.send(embed.setDescription("Bu komutu kullanabilmek için yetkin yok.")).then(x => x.delete({timeout: 5000}));
