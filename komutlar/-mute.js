...
if(!["764893496024629248","764893494296051723","764893503934300160","764893501329637396","794285242974208020"].some(role => message.member.roles.cache.get(role)) && (!message.member.hasPermission("ADMINISTRATOR"))) 
return message.channel.send(new MessageEmbed().setDescription(`${message.author} Komutu kullanmak için yetkin bulunmamakta.`).setColor('0x800d0d').setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
  
const mutelog = message.guild.channels.cache.find(c => c.id === '787910979198517249')//Mute logu
const muterol = message.guild.roles.cache.find(r => r.id === '764893543118405665')//Muteli rolü
